...
const initDatabase = async () => {
  ...
  try {
    ...
    await connection.execute(`CREATE DATABASE IF NOT EXISTS ${process.env.DB_NAME || 'ict_department'}`);
    console.log(`Database '${process.env.DB_NAME || 'ict_department'}' created or already exists`);

    // Switch to the database using query instead of execute
    await connection.query(`USE ${process.env.DB_NAME || 'ict_department'}`);
    ...
    const [dues] = await connection.execute('SELECT id FROM dues');
