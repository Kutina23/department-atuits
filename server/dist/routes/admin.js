...
router.get('/stats', async (req, res) => {
    try {
        const totalStudents = await User_1.User.count({ where: { role: 'student' } });
        const totalDues = await Due_1.Due.sum('amount') || 0;
        const paidPayments = await Payment_1.Payment.count();
        const pendingPayments = await StudentDue_1.StudentDue.count({ where: { status: ['Not Paid', 'Partial'] } });
...
router.get('/students', async (req, res) => {
    try {
        const students = await User_1.User.findAll({ where: { role: 'student' } });
        res.json(students);
    }
    catch (error) {
        res.status(500).json({ message: 'Server error' });
...
const getMatchingDuesForLevel = async (studentLevel) => {
    // Return empty array if student level is null or undefined
    if (!studentLevel) {
        return [];
    }
    const dues = await Due_1.Due.findAll();
...
const assignDuesToStudent = async (studentId, studentLevel) => {
    ...
    for (const due of matchingDues) {
        ...
        const existingStudentDue = await StudentDue_1.StudentDue.findOne({
            where: {
        ...
        if (!existingStudentDue) {
            await StudentDue_1.StudentDue.create({
                student_id: studentId,
                due_id: due.id,
                amount_paid: 0,
                balance: due.amount,
...
router.put('/students/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, index_number, level, email, phone, password } = req.body;
        const student = await User_1.User.findByPk(id);
        if (!student) {
        ...
        if ((oldLevel || '') !== (level || '')) {
            ...
            await StudentDue_1.StudentDue.destroy({ where: { student_id: id } });
...
router.delete('/students/:id', async (req, res) => {
    try {
        ...
        await Payment_1.Payment.destroy({ where: { student_id: id } });
        // Delete the student
...
const getStudentsForDueLevel = async (dueLevel) => {
    const students = await User_1.User.findAll({ where: { role: 'student' } });
...
router.post('/dues', async (req, res) => {
    try {
        ...
        const due = await Due_1.Due.create({ title, amount, level, academic_year, description });
...
router.put('/dues/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { title, amount, level, academic_year, description } = req.body;
        const due = await Due_1.Due.findByPk(id);
        if (!due) {
        ...
        if (amount !== due.amount) {
            const studentDues = await StudentDue_1.StudentDue.findAll({ where: { due_id: id } });
...
router.delete('/dues/:id', async (req, res) => {
    try {
        ...
        // Delete associated student dues first
        await StudentDue_1.StudentDue.destroy({ where: { due_id: dueId } });
        // Delete the due
        await Due_1.Due.destroy({ where: { id: dueId } });
...
router.get('/payments', async (req, res) => {
    try {
        const payments = await Payment_1.Payment.findAll({
            include: [
                { model: User_1.User, as: 'student', attributes: ['name'] },
                { model: Due_1.Due, as: 'due', attributes: ['title'] },
            ],
        });
    ...
    try {
        ...
        for (const student of students) {
            ...
            const studentDues = await StudentDue_1.StudentDue.findAll({
                ...
                include: [
                    { model: Due_1.Due, as: 'due', attributes: ['title', 'amount', 'academic_year'] }
                ]
            });
            ...
            let partialDues = 0;
            let unpaidDues = 0;
            const duesWithStatus = studentDues.map((studentDue) => {
...
router.delete('/events/:id', async (req, res) => {
    try {
        await Event_1.Event.destroy({ where: { id: req.params.id } });
...
router.get('/settings', async (req, res) => {
    try {
        ...
        const admin = await User_1.User.findByPk(adminId, {
