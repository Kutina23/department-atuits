...
router.use(auth_1.authenticateToken, (0, auth_1.authorizeRoles)('hod'));
// Get stats
...
router.post('/programs', async (req, res) => {
    try {
        ...
        const program = await Program_1.Program.create({
            name,
            duration,
            focus,
...
router.post('/faculty', upload_1.upload.single('image'), async (req, res) => {
    try {
        ...
        const faculty = await Faculty_1.Faculty.create({
            name,
            position,
            specialization,
            email,
...
router.put('/faculty/:id', upload_1.upload.single('image'), async (req, res) => {
    try {
        const { id } = req.params;
        const { name, position, specialization, email, phone, office, bio, qualifications, isActive } = req.body;
        const faculty = await Faculty_1.Faculty.findByPk(id);
        if (!faculty) {
